...
class TwitterAPIClient:
    ...
    def search_videos(self, search_query: str, max_results: int = 3) -> List[Dict[str, Any]]:
        ...
        try:
            self.logger.info(f"🔍 Searching Twitter: {search_query}")
            ...
            if response.status_code == 200:
                ...
                self.logger.error(f"❌ Twitter session error {response.status_code}: {response.text}")
                return []
                
        except requests.exceptions.ConnectionError:
            self.logger.error("❌ Twitter session service unreachable")
            return []
        except Exception as e:
            log_error_with_trace(self.logger, "❌ Twitter search failed", e)
            return []

@task(name="twitter-search-task")
def twitter_search_task(goal_id: str) -> Dict[str, Any]:
    """Simple Twitter search - find videos or don't"""
    logger = get_logger(__name__)  # ✅ CHANGE: Use centralized logger
...
def twitter_flow(goal_id: Optional[str] = None):
    ...
    if result["status"] == "success" and result.get("video_count", 0) > 0:
        ...
        try:
            ...
            log_error_with_trace(logger, "❌ Failed to trigger download", e)
