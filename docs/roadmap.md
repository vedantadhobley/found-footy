...
- **T/b — Auth + cookie lifecycle** [~2 days]
  ...
  - ✅ **T/b.3 + T/b.4 — VNC container + compose splits** (shipped 2026-07-22, MVP first per decisions.md 2026-07-22 reconciliation): `docker/twitter/Dockerfile` reconciled into one canonical image (Playwright base + playwright-go driver install + `WITH_VNC=true` gate for ~150 MB VNC binaries). Runtime mode via `TWITTER_VNC_MODE=true` env var — `docker/twitter/entrypoint.sh` boots Xvfb + fluxbox + x11vnc + websockify+noVNC before exec'ing the twitter binary. Playwright non-headless in the VNC container renders into the visible display (MVP path — if Twitter rejects login attempts from Playwright-instrumented Firefox, fall back to the Python raw-Firefox-subprocess trick per T/b.5 pending item). Top-level `/Dockerfile` reduced to worker/api/scaler-only with an explicit rejection guard on `BINARY=twitter`. `docker-compose.dev.yml` gained `twitter-vnc` service (opt-in via `profiles: [vnc]`, dedicated `twitter-vnc-profile` volume so visible-display Firefox's SQLite writes don't race the headless fleet). `docker-compose.prod.yml` fixed — twitter + twitter-vnc both switched from the broken top-level Dockerfile to the reconciled one; twitter was silently broken pre-reconciliation. Makefile targets: `twitter-vnc-up` / `twitter-vnc-down` / `twitter-vnc-logs` for one-command operator flow. Caddy config already had `found-footy-prod-twitter-vnc.luv`; dev needs a cross-repo rename (flagged separately for user approval).
  - ⏳ **T/b.5 — Ancillary hardening + confirmed-required Playwright-login fallback** — instance-scoped profile dir via hostname hash (multi-instance profile corruption); idle-CPU containment Firefox prefs (autoplay off, GIF animation off, ~20% CPU savings, twitter-spec §10); structured `auth_expired` log event for operator alerting (ship alongside T/c so alerting is in place before La Liga); hook `BackupCookies` invocation into the search success path (lands with T/c); ~~Playwright-login validation~~ **RESOLVED 2026-07-22** — experiment ran, Twitter blocked Playwright login at username entry ([decisions.md 2026-07-22](decisions.md)). Raw-Firefox-subprocess fallback (Python's `_launch_manual_firefox` at `session.py:313`) is confirmed required. Implementation deferred until either (a) current cookies expire or (b) T/c ships — half-day of subprocess-lifecycle + Playwright teardown/relaunch code, scope is clear.
- ✅ **T/c — Full search + scrape** (shipped 2026-07-23)
...
- **NOT reintroducing the Python-era Wikidata + LLM RAG** — user's call, docs/design/proposals/video-dedup.md notes this
