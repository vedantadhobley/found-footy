# Architectural decision archive through 2026-08-16

> **Frozen archive.** These decisions retain their original order and wording.
> New decisions are individual files routed by [`README.md`](./README.md).
> [`../decisions.md`](../decisions.md) preserves every historical heading and
> anchor for inbound-link compatibility.

---

## 2026-08-16 — Post-cutover authority moves from the rebuild plan to as-built truth

**Context:** the rebuild plan was prescriptive while the Go system was being
built. Production now differs from that 12,000-line target in workflow
topology, schemas, API/eventing, Twitter fleet ownership, alias behavior,
deployment, migration, and names. Requiring every agent to treat it as the
design bible causes stale intent to compete with shipped behavior.

**Decision:** current authority is, in order, code and schema; focused as-built
ledgers; the decision log; and the canonical issue register. Design proposals,
the rebuild plan, roadmaps, and dated audits are historical rationale and
evidence. An old audit claim must be verified against current code before it
becomes an active issue.

Project work starts at `docs/README.md`, follows its authority map, and reads
the accepted issue plus the relevant ledger and code. The Python archive and
functional spec remain behavior/parity evidence, not architecture templates.
A material change to a current contract still requires a decision and same-
change ledger update.

Historical material is not deleted merely for being old. DOC-007 first checks
its unique rationale and inbound links, then classifies it as retain, move,
consolidate, or delete. This keeps history available without letting it act as
a second backlog.

## 2026-08-16 — Firefox fleet ownership follows the Compose-selected network (FF-001)

**Incident:** dev and prod share one Docker daemon, but raw per-event Firefox
containers carried only `found-footy.fleet=firefox` plus an event ID. Both
workers therefore counted and listed one global fleet. Each reaper compared
that list against its own database and removed the other environment's
browsers. Stopping dev mitigated the incident but did not repair ownership.

**Decision:** the configured `FIREFOXFLEET_NETWORK` is the fleet's opaque
ownership scope. Compose still chooses the deployment partition; Go does not
branch on `dev` or `prod`. Every dynamic browser receives a scope label and a
daemon-global name containing the network. Count, list, and reaping require
the fleet label, scope label, and network. Start and removal additionally
inspect the event/scope labels and network membership before mutation.

Dynamic per-event browsers are created through the Docker API, not as Compose
service replicas, so Compose cannot automatically label or own them. Moving
them back under Compose would restore the fixed pool plus router/scaler model
that #160 deliberately replaced. The application therefore enforces ownership
for those children using the partition Compose supplied.

**Compatibility:** the daemon name changes from
`ff-firefox-ev-<event-prefix>` to
`ff-firefox-<network>-ev-<full-event-uuid>`, diverging from the signed-off #160
name in [`twitter-scaling.md`](../design/proposals/twitter-scaling.md). Each
container retains the old event-only name as a network-local alias. Existing
and replayed Temporal workflows keep resolving the same URL without a version
marker, while identical aliases remain isolated on the dev and prod networks.

An in-memory regression runs two fleet objects with the same event UUID against
one fake daemon and proves that provision, capacity, list, reap, and release
cannot cross the Compose-network boundary. A second test proves a same-named
container with foreign ownership labels is never adopted or removed.
...
contract now lives in the as-built [`docs/api.md`](../api.md) ledger.
...
[2026-08-05](../design/audits/audit-2026-08-05.md) G5 + the Firefox-leak dig).
...
removal) is #171. See [`design/audit-2026-08-05.md`](../design/audits/audit-2026-08-05.md)
...
+ share lookup for og-server. The shipped surface now lives in
[`api.md`](../api.md).
...
- [audit-2026-07-26.md](../design/audits/audit-2026-07-26.md) — filed this as P1; downgraded here to documented known-limitation
...
[`design-improvements-2026-07-23.md`](../design/audits/design-improvements-2026-07-23.md).
...
- [audit-2026-07-26.md](../design/audits/audit-2026-07-26.md) — P0 finding
- [design-improvements-2026-07-23.md #25](../design/audits/design-improvements-2026-07-23.md) — the "why drop" discussion (now resolved) + isCamelConcat/entity-ranking follow-ups
...
DiscoveryWorkflow (whose current successor is [`internal/workflow/event.go`](../../internal/workflow/event.go))
...
per [twitter-search-query.md D4](../design/proposals/twitter-search-query.md#d4)),
...
- [twitter-search-query.md](../design/proposals/twitter-search-query.md) — D4 (`max_age_minutes=3`), D1 (recall-first OR shape)
- [discovery.md](../design/proposals/discovery.md) — signed-off Discovery design (2026-07-16)
- [`internal/workflow/event.go`](../../internal/workflow/event.go) — current successor to the historical DiscoveryWorkflow
...
**Why**: [`design-audit.md`](../design/audits/design-audit.md) surfaced structural
...
since. See also [`design-audit.md`](../design/audits/design-audit.md) §16
...
[`design-audit.md`](../design/audits/design-audit.md) §3's verdict ("keep Mongo"),
...
**Consequence for [`design-audit.md`](../design/audits/design-audit.md) §6**: the
