...
| [`proposals/video-dedup.md`](./proposals/video-dedup.md) | Phase proposal | ⏳ **AWAITING REVIEW** | O4/O5 design — full video pipeline redesign (metadata hard-filter → URL → content hash → vision #1 → perceptual → vision #2 quality comparison → upload). Binary verified/unverified categories. Per-event AssetWorkflow with queue-drain completion (kills Python's 5-min tail). Popularity derived from video_shares count. Cross-event pg dedup. Extensible event_tweets table. 7 open questions on perceptual hash algo + LSH prefix + threshold + rubrics. Blocks V/a; V/a blocks on T shipping. |
| [`proposals/twitter-port.md`](./proposals/twitter-port.md) | Phase proposal | ⏳ **AWAITING REVIEW** | Phase T design — port Python `twitter/` service to Go. Preserves dual-mode auth, cookie backup, URL exclusion, time-based scroll; improves rate-limit handling, error taxonomy, session recovery, browser-crash watchdog, download-extract split. Browser library choice (Playwright-Go with Firefox my lean) + 7 more open questions. T sequenced right after O3, before O4. |
