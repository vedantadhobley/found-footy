...
| EventWorkflow | ✓ renamed from DiscoveryWorkflow 2026-08-03 (Option 2, #164); consumer in build | Spawned by Monitor's `ReconcileFixture` via `DownstreamSpawner` (workflow ID `event-{id}`) when `downstream_triggered` flips (2026-07-16 — Temporal-direct spawn, not NATS). **Currently runs the discovery phase** (search + candidate collection, ex-`DiscoveryWorkflow`); the producer/Selector consumer (Video children → dedup→vision→promote→rank) lands in #164c. pg `workflow_type` value stays `'discovery'` (internal label). See [`design/v-phase-orchestration.md`](design/v-phase-orchestration.md). | `internal/workflow/event.go` |
