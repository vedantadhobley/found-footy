...
- [`docs/rebuild/proposals/`](./docs/rebuild/proposals/) — **design-first drafts for phases before they're committed.** Look here before proposing designs — the next phase may already have an open proposal (currently: `discovery.md` for O3 SIGNED OFF 2026-07-16, O3/a unblocked; `twitter-port.md` for phase T AWAITING REVIEW with 8 open questions on browser library + auth flow + CDN split; `video-dedup.md` for O4/O5 AWAITING REVIEW with 7 open questions on hash algo + rubrics; `monitor.md` SUPERSEDED — historical only). `workflow-audit-2026-07-09.md` and `api-football-audit-2026-07-09.md` are cross-cutting audits, not phase proposals.
