...
func TestIsFresh_UnresolvedIsNeverFresh(t *testing.T) {
	if makeClub().IsFresh(time.Now(), 30*24*time.Hour) {
		t.Error("IsFresh should be false for an unresolved placeholder")
	}
}

func TestIsFresh_JustResolvedIsFresh(t *testing.T) {
	ta := makeClub()
	now := time.Date(2026, 7, 19, 12, 0, 0, 0, time.UTC)
	ta.SetResolution("Q8701", []string{"atletico"}, now)
	if !ta.IsFresh(now, 30*24*time.Hour) {
		t.Error("IsFresh should be true right after SetResolution")
	}
}

func TestIsFresh_ExpiredResolutionIsStale(t *testing.T) {
	ta := makeClub()
	resolvedAt := time.Date(2026, 6, 1, 12, 0, 0, 0, time.UTC)
	ta.SetResolution("Q8701", []string{"atletico"}, resolvedAt)
	// 45 days later, TTL of 30 days → stale
	now := resolvedAt.Add(45 * 24 * time.Hour)
	if ta.IsFresh(now, 30*24*time.Hour) {
		t.Error("IsFresh should be false 45 days after resolution with 30-day TTL")
	}
}

func TestIsFresh_EdgeCaseAtTTLBoundary(t *testing.T) {
	ta := makeClub()
	resolvedAt := time.Date(2026, 7, 1, 12, 0, 0, 0, time.UTC)
	ta.SetResolution("Q8701", []string{"atletico"}, resolvedAt)
	// Exactly at TTL: current implementation uses `<`, so boundary = stale.
	if ta.IsFresh(resolvedAt.Add(30*24*time.Hour), 30*24*time.Hour) {
		t.Error("IsFresh should treat exactly-TTL-elapsed as stale (uses strict <)")
	}
	// One second before boundary: fresh.
	if !ta.IsFresh(resolvedAt.Add(30*24*time.Hour-time.Second), 30*24*time.Hour) {
		t.Error("IsFresh should be true one second before TTL boundary")
	}
}
