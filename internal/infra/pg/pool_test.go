...
// TestVerifySchema_StampMatchDrift exercises the P0-3 drift guard end to end:
// first run stamps the schema.sql fingerprint, a second run matches, and a
// tampered stored hash makes the guard refuse — the silent-drift footgun
// turned loud. The testcontainer applies schema.sql via WithInitScripts, so
// its fingerprint equals pg.SchemaHash().
func TestVerifySchema_StampMatchDrift(t *testing.T) {
	if testing.Short() {
		t.Skip("integration test skipped in -short mode")
	}
	ctx, cancel := context.WithTimeout(context.Background(), 2*time.Minute)
	...
	// 1. First run on a fresh DB stamps the baseline.
	if err := pool.VerifySchema(ctx); err != nil {
		t.Fatalf("first VerifySchema (stamp): %v", err)
	}
	if !fx.log.HasAction(vocabulary.ModuleInfraPG, vocabulary.ActionSchemaStamped) {
		t.Errorf("expected ActionSchemaStamped on first run; got: %+v", fx.log.Snapshot())
	}
	var stored string
	if err := pool.QueryRow(ctx, "SELECT schema_hash FROM schema_version WHERE id = 1").Scan(&stored); err != nil {
		t.Fatalf("read stamped hash: %v", err)
	}
	if stored != pg.SchemaHash() {
		t.Fatalf("stamped hash = %q, want %q", stored, pg.SchemaHash())
	}

	// 2. Second run: fingerprint matches → no error, no re-stamp.
	if err := pool.VerifySchema(ctx); err != nil {
		t.Fatalf("second VerifySchema (match): %v", err)
	}

	// 3. Tamper the stored fingerprint → the guard must refuse to proceed.
	if _, err := pool.Exec(ctx, "UPDATE schema_version SET schema_hash = 'deadbeefdeadbeef' WHERE id = 1"); err != nil {
		t.Fatalf("tamper: %v", err)
	}
	if err := pool.VerifySchema(ctx); err == nil {
		t.Fatal("VerifySchema returned nil on drift — the footgun stayed silent")
	}
	if !fx.log.HasAction(vocabulary.ModuleInfraPG, vocabulary.ActionSchemaDrift) {
		t.Errorf("expected ActionSchemaDrift emission on mismatch; got: %+v", fx.log.Snapshot())
	}
}
