...
// ListLiveFleetEventIDs projects browser ownership from the existing lifecycle.
// Before discovery registration, a positive debounce counter on an active fixture
// owns warmup, including the trigger-to-checklist crash/retry handoff. Afterwards,
// an unfinished downstream row owns the browser regardless of fixture state.
// Completed discovery alone cannot retain a failed-release browser until full time.
// Removed events never own browsers; anonymous zero-counter placeholders do not
// own warmup. Pending rows remain conservative ownership, not a Temporal liveness test.
func (r *EventRepo) ListLiveFleetEventIDs(ctx context.Context) ([]uuid.UUID, error) {
	rows, err := r.pool.Query(ctx, `
		...
		WHERE e.removed = false
		  AND (
		      (f.state = 'active' AND e.debounce_count > 0
		       AND NOT EXISTS (
		           SELECT 1 FROM event_downstream_workflows edw
		           WHERE edw.event_id = e.id AND edw.workflow_type = 'discovery'
		       ))
		      OR EXISTS (
		          SELECT 1 FROM event_downstream_workflows edw
